...
12. Experiment with adding warmup schedulers, changing optimisers, LRs, weight_decays, batch_sizes, etc     -   [ ] 
    ...
    3. Try accumulating steps to avoid OOM after increasing batch size to 64ish and learning rate to 6e-4ish   [x]
    4. Linear Warmup before Cosine using LR=4e-4 using sequential scheduler [x]
    5. Add label-smoothing to CrossEntropyLoss                                                                      [x]
    6.  Use WeightedRandomSampler with unweighted Cross Entropy for sample-based criterion                             [x]
    7.  Change criterion to treat problem as a single class problem by using MSELoss criterion.          [x]
    8.  Changing MSELoss to class weighted MSELoss
